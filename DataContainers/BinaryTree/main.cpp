...
void main()
{
	...
	cout << "Введите размер дерева: "; cin >> n;
	Tree tree;
	t_start = clock();	//Функция 'clock()' возвращает количество тактов CPU
	for (int i = 0; i < n; i++)
	{
		tree.insert(rand() % 100);
	}
	...
	t_start = clock();
	cout << "Сумма элементов в дереве:\t" << tree.sum() << tab;
	t_end = clock();
	cout << double(t_end - t_start) / CLOCKS_PER_SEC << " сек." << endl;

	t_start = clock();
	cout << "Среднее-арифметическое элементов в дереве: " << tree.avg() << tab;
	t_end = clock();
	cout << double(t_end - t_start) / CLOCKS_PER_SEC << " сек." << endl;
